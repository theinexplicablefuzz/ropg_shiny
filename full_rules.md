...
#### Stunts
Stunts allow characters to combine multiple actions into a single action. This could be attacking multiple targets, attacking multiple times in succession, or combining multiple effects into a single spell. A stunt requires n(n-1)/2 successes where n = the number of distinct effects. 

Two or more characters can communicate a plan effectively can attempt a stunt together. In this case divide the effects among each character in a way that is reasonable and each character must achieve successes according to the number of effects assigned and one of the participating characters must succeed on communicating the stunt. 

#### Successes Per Character
| Effects | 1 Character | 2 Characters      | 3 Characters   | 4 Characters |
|:-------:|:-----------:|:-----------------:|:--------------:|:------------:|
| 1       | 1           | NA                | NA             | NA           |
| 2       | 3           | 2,1               | NA             | NA           |
| 3       | 6           | 4,1 or 3,2        | 2,1,1          | NA           |
| 4       | 10          | 4,3 or 7,1 or 6,2 | 4,1,1 or 3,2,1 | 2,1,1,1      |
| n       | n(n-1)/2    | ...               | ...            | ...          |

#### Knacks
Knacks allow characters to turn perform actions and stunts reliably. Provided a character has an effective ability rank of 3+ when performing an action or stunt, a character can internalize the action or stunt allowing them to automatically succeed on future stunts by spending 1 experience point per required success. For example, a knack can be used to gain proficiency in a weapon, and going forward the character would automatically succeed on attacks with sufficiently similar weapons. Knacks for multi-effect stunts give proficiency in each individual effect separately. In addition, single effect knacks can be expanded into multi-effect knacks. Stunts involving multiple characters can also be turned into knacks in this way. The character must meet the 3+ effective ability rank requirement for each effect of a stunt to turn it into a knack.
